...
1. Define a scoring rubric from the user's actual constraints. Include customer pain, willingness to pay, reachability, competition, differentiation, build/operating burden, risk, and fit.
2. Generate a diverse candidate set. Discovery-and-analysis may be used for divergent ideation, but do not treat its rankings as market evidence.
...
4. Shortlist 3–5 candidates before spending heavily on research.
...
8. Revise the product concept in response to evidence. A competitor can validate demand while invalidating an undifferentiated implementation.
9. Score finalists only after research. Explain major score changes from the initial screen.
10. Return a clear verdict for each finalist: `pursue`, `modify`, `hold`, or `reject`.
...
If evidence is thin, label confidence low and recommend research or a test—not confidence disguised as a score.

## Output

Lead with the verdict and the best-supported opportunity. Then provide:
...
Prefer a small comparison table when several finalists use the same fields. Keep discarded ideas brief unless the user asks for the full funnel.
